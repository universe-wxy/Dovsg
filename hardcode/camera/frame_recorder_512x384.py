...
real_height = 384  # remove bottom 200px to remove robot arm in image bottom
real_width = 512

class RecorderImage():
    def __init__(self, serial_number="309122301398", 
                 WH=[640, 480], FPS=30, depth_threshold=[0, 2.5]):
    ...
    def get_one_align_frame(self, retry=10):
        for i in range(retry):
            try:
                ...
                point = point[:384, 64:576]
                color = color[:384, 64:576]
                depth = depth[:384, 64:576]
...
if __name__ == "__main__":
    ...
    is_ok, depth_image, color_image, calib = record.get_one_align_frame()
    if is_ok:
        print(depth_image.shape, color_image.shape, calib.shape)
