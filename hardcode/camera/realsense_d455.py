...
class RealSense_D455:
    def __init__(self, WH=[640, 480], remove_robot=False, depth_threshold=[0, 2], serial_number="309122301398", FPS=30):
        ...
        self.config.enable_stream(rs.stream.depth, WH[0], WH[1], rs.format.z16, FPS)
        self.config.enable_stream(rs.stream.color, WH[0], WH[1], rs.format.bgr8, FPS)
        ...
        if self.remove_robot and WH == [640, 480]:
            # change heigh and width from 480 x 640 to 384 x 512
    ...
    def get_observations(self):
        ...
        if self.remove_robot:
            if self.WH == [1280, 720]:
                ...
                depths = depths[:600, :]
                mask = mask[:600, :]
            elif self.WH == [640, 480]:
                points = points[:384, 64:576]
                colors = colors[:384, 64:576]
                depths = depths[:384, 64:576]
                mask = mask[:384, 64:576]
            else:
                assert 1 == 0        
